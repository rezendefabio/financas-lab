...
import com.laboratorio.financas.emprestimo.application.BuscarEmprestimoPorIdUseCase;
...
public class EmprestimoController {
    ...
    private final BuscarEmprestimoPorIdUseCase buscarUseCase;
    ...
    public EmprestimoController(CriarEmprestimoUseCase criarUseCase,
                                ...
                                BuscarEmprestimoPorIdUseCase buscarUseCase,
    ...
    @GetMapping("/{id}")
    public EmprestimoResponse buscar(@PathVariable UUID id, Authentication authentication) {
        resolverUserId(authentication);
        return EmprestimoResponse.fromDomain(buscarUseCase.executar(id));
    }
    ...
    public EmprestimoResponse criar(
        ...
        Emprestimo criado = criarUseCase.executar(comando);
        EmprestimoResponse response = EmprestimoResponse.fromDomain(criado);
    ...
    public EmprestimoResponse atualizar(
            ...
            @RequestHeader(value = "X-Screen-Code", required = false) String screenCode) {
        resolverUserId(authentication);
        Emprestimo antes = buscarUseCase.executar(id);
        String before = toJson(EmprestimoResponse.fromDomain(antes));
        ...
        Emprestimo atualizado = atualizarUseCase.executar(comando);
        EmprestimoResponse response = EmprestimoResponse.fromDomain(atualizado);
        auditPublisher.publish(new AuditEvent(
