...
public class LembreteRepositoryImpl implements LembreteRepository {
    ...
    public LembreteRepositoryImpl(LembreteJpaRepository jpaRepository, LembreteMapper mapper) {
    ...
    public List<Lembrete> listarPorUserId(UUID userId) {
        return jpaRepository.findByUserIdOrderByDataLembreteAsc(userId).stream()
                .map(mapper::toDomain)
                .toList();
    }
