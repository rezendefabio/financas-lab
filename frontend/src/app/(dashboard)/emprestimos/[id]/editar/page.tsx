...
import type { AtualizarEmprestimoPayload } from '@/features/emprestimo/types/emprestimo'
import { Card, CardContent } from '@/shared/components/ui/card'
import { Button } from '@/shared/components/ui/button'
import { Skeleton } from '@/shared/components/ui/skeleton'

function toAtualizarPayload(v: EmprestimoFormValues): AtualizarEmprestimoPayload {
  return {
    descricao: v.descricao,
    nomeTerceiro: v.nomeTerceiro?.trim() ? v.nomeTerceiro.trim() : undefined,
    tipo: v.tipo,
    valor: v.valor,
    moeda: v.moeda,
...
export default function EditarEmprestimoPage() {
  ...
  const mutation = useMutation({
    mutationFn: (v: EmprestimoFormValues) =>
      emprestimoService.atualizar(id, toAtualizarPayload(v)),
    onSuccess: async () => {
