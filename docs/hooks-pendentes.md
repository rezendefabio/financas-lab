...
- **Captura automatica de metricas de execucao** (Sub-etapa fix-metricas-automaticas, PR #284). Hook nativo Claude Code (`PostToolUse`) em `.claude/hooks/post-tool-use/metrics-capture.ps1`, referenciado por `.claude/settings.json` (gitignored, gerado pelo `setup.ps1`) como segundo matcher do array `PostToolUse` (matcher `Bash`, timeout 10s). Le o JSON do stdin e, para comandos caros (`mvn verify`, `mvn test`, `npm run test:run/build/lint`, `gh pr create`, `git push`), grava uma entrada JSON Lines em `.claude/metrics.log` com `ts`, `step`, `cmd`, `branch` e `duracao_ms` (null se `tool_response` nao expoe o dado). **Non-blocking por design** (`$ErrorActionPreference = "SilentlyContinue"` + `exit 0` em todos os caminhos: stdin vazio, JSON invalido, comando nao reconhecido, falha em git). Complementado pela instrumentacao de `scripts/check.ps1` e `scripts/check-front.ps1` (gravam timing + `exit_code` ao fim do gate, logica do gate inalterada) e pelo visualizador `scripts/show-metrics.ps1`. O log `.claude/metrics.log` e gitignored. Motivacao: a coleta de metricas via instrucao em prosa no prompt falhava porque competia com o objetivo principal do executor -- a coleta precisa ser mecanica.
